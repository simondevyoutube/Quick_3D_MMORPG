  ...
  class UIController extends Component {
    ...
    AddEventMessages(events) {
      for (let e of events) {
        ...
        const attackerName = e.attacker.Name == "player"
          ? "You"
          : e.attacker.Account.name;
        const targetName = e.target.Name == "player"
          ? "you"
          : e.target.Account.name;

        this.AddChatMessage({
          name: "",
          text: attackerName + " hit " + targetName + " for " + e.amount +
            " damage!",
          action: true,
        });
      }
