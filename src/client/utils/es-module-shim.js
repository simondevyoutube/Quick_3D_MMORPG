...
(function() {
  ...
  function objectAssign(to, from) {
    for (const p in from) to[p] = from[p];
    return to;
  }
  ...
  function parse(Q, B = '@') {
    ...
    if (
      (C > 0 && A.memory.grow(Math.ceil(C / 65536)),
      (function(A, Q) {
        const B = A.length;
        let C = 0;
        for (; C < B; ) Q[C] = A.charCodeAt(C++);
      })(Q, new Uint16Array(A.memory.buffer, A.sa(Q.length), Q.length + 1)),
      !A.parse())
    )
      throw Object.assign(
        new Error(
          `Parse error ${B}:${Q.slice(0, A.e()).split('\n').length}:${A.e() -
            Q.lastIndexOf('\n', A.e() - 1)}`
        ),
        { idx: A.e() }
      );
    const E = [];
    const g = [];
    for (; A.ri(); ) E.push({ s: A.is(), e: A.ie(), d: A.id() });
  ...
  class WorkerShim {
    constructor(aURL, options = {}) {
      ...
      const workerScriptUrl = createBlob(
        `importScripts('${esModuleShimsSrc}');importShim.map=${JSON.stringify(
          options.importMap
        )};importShim('${
          new URL(aURL, baseUrl).href
  ...
  function resolveDeps(load, seen) {
    ...
    const lastNonEmptyLine = resolvedSource.slice(
      resolvedSource.lastIndexOf('\n') + 1
    );
    load.b = lastLoad = createBlob(
      resolvedSource +
        (lastNonEmptyLine.startsWith('//# sourceMappingURL=')
          ? '\n//# sourceMappingURL=' +
            resolveUrl(lastNonEmptyLine.slice(21), load.r)
          : '') +
        '\n//# sourceURL=' +
        load.r
    );
    ...
    load.f = (async () => {
      if (!source) {
        ...
        if (contentType.match(/^(text|application)\/(x-)?javascript(;|$)/)) {
          ...
          source = `const s=new CSSStyleSheet();s.replaceSync(${JSON.stringify(
            await res.text()
          )});export default s`;
        } else if (contentType.match(/^application\/wasm(;|$)/)) {
          const module = (wasmModules[url] = await WebAssembly.compile(
            await res.arrayBuffer()
          ));
          const deps = WebAssembly.Module.imports
            ? WebAssembly.Module.imports(module).map(impt => impt.module)
            : [];

          const aDeps = [];
          load.a = [
            aDeps,
            WebAssembly.Module.exports(module).map(expt => expt.name),
          ];
    ...
    load.L = load.f.then(async deps => {
      load.d = await Promise.all(
        deps.map(async depId => {
          const depLoad = getOrCreateLoad(
            await resolve(depId, load.r || load.u)
          );
          await depLoad.f;
          return depLoad;
        })
      ...
      if (hasDocument)
        for (const script of document.querySelectorAll(
          ...
          importMapPromise = importMapPromise.then(async () => {
            importShim.map = resolveAndComposeImportMap(
              script.src
