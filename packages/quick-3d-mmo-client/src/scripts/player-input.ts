...
class BasicCharacterControllerInput extends Component {
  ...
  _onMouseUp(event) {
    const rect = document.getElementById(DOM_IDS.THREEJS).getBoundingClientRect();
    const pos = {
    ...
    // hack
    document.getElementById(DOM_IDS.QUEST_UI).style.visibility = 'hidden';
    ...
    for (let p of pickables) {
      ...
      if (ray.intersectsBox(box)) {
        p.Broadcast({
          topic: EVENT_TYPES.INPUT_PICKED
        });
        break;
      }
    }
