...
for (let k in multiples) {
    for (let i = 2; i < multiples[k].num; ++i) {
        _SCENERY[multiples[k].key + i] = Object.assign({}, _SCENERY[k]);
...
class SceneryController extends Component {
    ...
    SpawnAt_(biome, spawnPos) {
        ...
        const q = new THREE.Quaternion().setFromAxisAngle(new THREE.Vector3(0, 1, 0), this.noise_.Get(spawnPos.x, 5.0, spawnPos.z) * 360);
        e.SetQuaternion(q);
        return e;
    }
    SpawnCrap_() {
