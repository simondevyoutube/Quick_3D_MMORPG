...
class UIController extends Component {
  _params: any;
  _quests: {};
  iconBar_: { stats: HTMLElement; inventory: HTMLElement; quests: HTMLElement; };
  _ui: { inventory: HTMLElement; stats: HTMLElement; quests: HTMLElement; };
  ...
  InitComponent() {
    ...
    this.iconBar_.inventory.onclick = (m) => { this.OnInventoryClicked_(m); };
    this.iconBar_.stats.onclick = (m) => { this.OnStatsClicked_(m); };
    this.iconBar_.quests.onclick = (m) => { this.OnQuestsClicked_(m); };
    this.HideUI_();

    this.chatElement_ = document.getElementById('chat-input') as HTMLInputElement;
    this.chatElement_.addEventListener(
      'keydown', (e) => this.OnChatKeyDown_(e), false);
  }

  FadeoutLogin() {
    const loginElement = document.getElementById(DOM_IDS.LOGIN_UI);
  ...
  OnChatKeyDown_(evt) {
    if (evt.keyCode === 13) {
      ...
      if (msg != '') {
        const net = this.FindEntity(KNOWN_ENTITIES.NETWORK).GetComponent(
          NAMED_COMPONENTS.NETWORK_CONTROLLER);
        net.SendChat(msg);
      }
