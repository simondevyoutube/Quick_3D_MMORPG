...
class NetworkEntityController extends Component {
  ...
  InitComponent() {
    ...
    this._RegisterHandler(
      EVENT_TYPES.NETWORK_UPDATE, (m) => { this.OnUpdate_(m); });
    this._RegisterHandler(
      EVENT_TYPES.ACTION_ATTACK, (m) => { this.OnActionAttack_(m); });
  }

  InitEntity() {
    this.net_ = this.FindEntity(KNOWN_ENTITIES.NETWORK).GetComponent(
