...
class NetworkController extends Component {
    ...
    SetupSocket_() {
        ...
        this.socket_.on(EVENT_TYPES.CONNECT, () => {
            ...
            this.socket_.emit(EVENT_TYPES.LOGIN_COMMIT, document.getElementById('login-input').value);
        });
    ...
    OnMessage_(e, d) {
        if (e == EVENT_TYPES.WORLD_PLAYER) {
            const spawner = this.FindEntity(KNOWN_ENTITIES.SPAWNERS).GetComponent(NAMED_COMPONENTS.PLAYER_SPAWNER);
            ...
            const updates = d;
            const spawner = this.FindEntity(KNOWN_ENTITIES.SPAWNERS).GetComponent(NAMED_COMPONENTS.NETWORK_ENTITY_SPAWNER);
