...
const { ANIM_TYPES, EVENT_TYPES, KNOWN_ENTITIES, NAMED_COMPONENTS, STATE_TYPES, CHARACTER_MODELS } = Object.assign(Object.assign({}, Constants), Defs);
...
class BasicCharacterController extends Component {
    ...
    _FindIntersections(pos, oldPos) {
        ...
        for (let i = 0; i < nearby.length; ++i) {
            ...
            const d = Math.pow((Math.pow((pos.x - e.Position.x), 2) + Math.pow((pos.z - e.Position.z), 2)), 0.5);
            // HARDCODED
            if (d <= 4) {
                const d2 = Math.pow((Math.pow((oldPos.x - e.Position.x), 2) + Math.pow((oldPos.z - e.Position.z), 2)), 0.5);
